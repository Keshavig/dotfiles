...
void applyrules(Client *c) {
  ...
  // Ensure floating windows get the border width applied
  // if (c->isfloating) c->bw = borderpx;
}
...
void manage(Window w, XWindowAttributes *wa) {
  ...
  // Ensure floating window also get border
  if (c->isfloating)
    c->bw = borderpx;
...
//     mw = m->ww - m->gappih;
//
//   for (i = 0, my = m->gappih, c = nexttiled(m->clients); c;
//        c = nexttiled(c->next), i++) {
//     if (i < m->nmaster) {
//       h = (m->wh - my) / (MIN(n, m->nmaster) - i) - m->gappih;
//       resize(c, m->wx + m->gappih, m->wy + my, mw - (2 * c->bw) - m->gappih,
//              h - (2 * c->bw), 0);
...
//       resize(c, m->wx + mw + (ox * stairpx) + m->gappih,
//              m->wy + (oy * stairpx) + m->gappih,
//              m->ww - mw - (2 * c->bw) - (ow * stairpx) - (2 * m->gappih),
//              m->wh - (2 * c->bw) - (oh * stairpx) - (2 * m->gappih), 0);
//     }
//   }
// }
